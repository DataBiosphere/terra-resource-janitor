...
public class JanitorDao {
    ...
    /**
     * Returns up to {@code limit} {@link TrackedResource}s that are ready to be scheduled for cleaning.
     */
    @Transactional(propagation = Propagation.REQUIRED, isolation = Isolation.SERIALIZABLE)
    public List<TrackedResource> retrieveSchedulableResources(Instant now, int limit) {
        String sql = "SELECT id, resource_type, resource_uid, creation, expiration, state FROM tracked_resource " +
    ...
    public void initiateFlight(UUID trackedResourceId, String flightId) {
      ...
      MapSqlParameterSource params = new MapSqlParameterSource().addValue("cleaning", TrackedResourceState.CLEANING.toString())
              .addValue("ready", TrackedResourceState.READY.toString())
              .addValue("id", trackedResourceId);
      jdbcTemplate.update(sql, params);
    }
      {
        String sql = "INSERT INTO flight (tracked_resource_id, flight_id, state) values (:tracked_resource_id, :flight_id, :initiating)";
  ...
  public void setFlightState(String flightId, FlightState flightState) {
    ...
    private static final RowMapper<TrackedResource> TRACKED_RESOURCE_ROW_MAPPER = new RowMapper<TrackedResource>() {
        @Override
